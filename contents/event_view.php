...
<head>
    ...
    <meta property="og:title" content="언리얼 엔진 5 챌린지 발표">
    <meta property="og:description" content="멋진 상상력이 담긴 언리얼 엔진 5 챌린지 당첨을 축하드립니다.">
    <meta property="og:image" content="https://unrealsummit16.cafe24.com/2023/ch_result.jpg">
    <meta property="og:url" content="https://unrealsummit16.cafe24.com/2023/ch_result.jpg">
    <title>에픽 라운지</title>
...
<?
	$RData = sql_fetch("select * from v3_rsc_event_bbs where rsc_bbs_idx = '{$rsc_bbs_idx}'");
...
<?
		...
		<div class="board_ot_list">
			...
			<ul>
			<?
			$other_result = sql_query("select * from v3_rsc_event_bbs  where display_yn='Y' and rsc_bbs_idx<>{$rsc_bbs_idx} order by abs({$rsc_bbs_idx} - rsc_bbs_idx) and display_yn='Y' asc,rsc_bbs_idx DESC limit 3 ");
			...
			?>
				<li>
					<a href="./event_view.php?rsc_bbs_idx=<?=$other["rsc_bbs_idx"]?>&status=<?=$_GET[status]?>&category=<?=$_GET[category]?>">
		...
		<div class="board_btn_box">
			<a href="./event_list.php?status=<?=$_GET[status]?>&category=<?=$_GET[category]?>" ><span class="text">목록으로</span><span class="icon"></span></a>
		</div>
	</div>
	</div>
</div>
