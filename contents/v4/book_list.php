...
$seo_title       = $v3_seo['seo_title']       ?: '에픽 라운지 | 백서/eBook';
...
// 산업분야
$sql = "SELECT DISTINCT cate_industry FROM v3_rsc_book_bbs WHERE cate_industry != '' AND display_yn = 'Y' ORDER BY cate_industry";
...
// 제품
$sql = "SELECT DISTINCT cate_product FROM v3_rsc_book_bbs WHERE cate_product != '' AND display_yn = 'Y' ORDER BY cate_product";
$result = sql_query($sql);
while ($row = sql_fetch_array($result)) {
...
// 주제
$sql = "SELECT DISTINCT cate_subject FROM v3_rsc_book_bbs WHERE cate_subject != '' AND display_yn = 'Y' ORDER BY cate_subject";
$result = sql_query($sql);
while ($row = sql_fetch_array($result)) {
...
// 난이도
$sql = "SELECT DISTINCT cate_difficult FROM v3_rsc_book_bbs WHERE cate_difficult != '' AND display_yn = 'Y' ORDER BY cate_difficult";
$result = sql_query($sql);
while ($row = sql_fetch_array($result)) {
...
// 전체 건수
$total_count = (int)sql_fetch("SELECT COUNT(*) as cnt FROM v3_rsc_book_bbs {$where}")['cnt'];

// 목록 조회
$sql = "SELECT * FROM v3_rsc_book_bbs {$where} ORDER BY ordr DESC, rsc_bbs_idx DESC" . v4_limit($page, $per_page);
$result = sql_query($sql);
$items = [];
while ($row = sql_fetch_array($result)) {
    $items[] = $row;
...
<head>
    ...
    <?php endif; ?>
    <meta property="og:url" content="https://epiclounge.co.kr/contents/v4/book_list.php">
                ...
                <div id="card-list" class="v4-card-grid v4-card-grid--gallery">
                    <?php if (empty($items)): ?>
                        <div class="v4-empty">
                            ...
                            <div class="v4-empty__title">등록된 백서가 없습니다.</div>
                            <div class="v4-empty__desc">새로운 백서가 곧 등록될 예정입니다.</div>
                        </div>
                    <?php else: ?>
                        <?php foreach ($items as $item): ?>
                            <?php echo render_resource_card($item, 'book', $view_url, 'book'); ?>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
...
(function($) {
    ...
    function loadContent(append) {
        ...
        $.ajax({
            url: '/v3/contents/v4/ajax/book.ajax.php',
            type: 'POST',
            dataType: 'json',
