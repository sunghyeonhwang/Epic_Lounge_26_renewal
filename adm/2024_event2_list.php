...
$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_track <> '쿠폰' and apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'NORMAL%'  and (apply_product_code like '%ALL')";
$track_nor_all = sql_fetch($sql);

$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_track <> '쿠폰' and apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'STD%'  and ( apply_product_code like '%ALL')";
$track_std_all = sql_fetch($sql);

$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_track <> '쿠폰' and apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'NORMAL%'  and (apply_product_code like '%28')";
$track_nor_28 = sql_fetch($sql);

$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_track <> '쿠폰' and apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'STD%'  and (apply_product_code like '%28')";
$track_std_28 = sql_fetch($sql);


$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_track <> '쿠폰' and apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'NORMAL%'  and (apply_product_code like '%29')";
$track_nor_29 = sql_fetch($sql);

$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_track <> '쿠폰' and apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'STD%'  and (apply_product_code like '%29')";
$track_std_29 = sql_fetch($sql);


$row_ticket = sql_fetch(" select sum(date1) date1 ,sum(date2) date2  from 2024_event_ticket where name <> '쿠폰'  ");
...
?>
    ...
    <form name="fsearch" id="fsearch" class="local_sch01 local_sch" method="get">
        ...
        $offline_txt .= "28 일권 ".$track_nor_28["cnt"]." / ";
        $offline_txt .= "29 일권 ".$track_nor_29["cnt"]." / ";
        $offline_txt .= "학생 양일 ".$track_std_all["cnt"]." / ";
        $offline_txt .= "학생 28일 ".$track_std_28["cnt"]." / ";
        $offline_txt .= "학생 29일 ".$track_std_29["cnt"].") ";

        ?>
	<div class="local_ov01 local_ov">
        <span style="font-size: 18px">
        <strong>오프라인</strong>:
        <br>28일권 <strong><?=($track_nor_28["cnt"] + $track_nor_all["cnt"] + $track_std_28["cnt"] + $track_std_all["cnt"])?>명</strong> / <?=number_format($row_ticket["date1"])?>명 (일반:<?=$track_nor_28["cnt"] + $track_nor_all["cnt"]?>명, 학생:<?=$track_std_28["cnt"] + $track_std_all["cnt"]?>명) | <span style="color: #535353">D1. 잔여석: <?=$row_ticket["date1"]-($track_nor_28["cnt"] + $track_nor_all["cnt"] + $track_std_28["cnt"] + $track_std_all["cnt"])?>석</span>

        <br>29일권 <strong><?=($track_nor_29["cnt"] + $track_nor_all["cnt"] + $track_std_29["cnt"] + $track_std_all["cnt"])?>명</strong> / <?=number_format($row_ticket["date2"])?>명 (일반:<?=$track_nor_29["cnt"] + $track_nor_all["cnt"]?>명, 학생:<?=$track_std_29["cnt"] + $track_std_all["cnt"]?>명) | <span style="color: #535353">D2. 잔여석:  <?=$row_ticket["date2"] - ($track_nor_29["cnt"] + $track_nor_all["cnt"] + $track_std_29["cnt"] + $track_std_all["cnt"])?>석</span>
        <br><br></span>

        <a href="2024_event2_list.php" class="btn btn_03 ft_11">전체 <?=$row_all_cnt["cnt"]?>명</a>
        <a href="2024_event2_list.php?sc_md=pay" class="btn <?=$sc_md == "pay" ? "btn_03":"btn_02"?> ft_11">오프라인 <?=$row_pay_cnt["cnt"]?>명</a>
        <a href="2024_event2_list.php?scode=NORMAL_ALL" class="btn  <?=$scode == "NORMAL_ALL" ? "btn_03":"btn_02"?>  ft_11"><?="양일권 ".$track_nor_all["cnt"]?>명</a>
        <a href="2024_event2_list.php?scode=NORMAL_28" class="btn  <?=$scode == "NORMAL_28" ? "btn_03":"btn_02"?>  ft_11"><?="28 일권 ".$track_nor_28["cnt"]?>명</a>
        <a href="2024_event2_list.php?scode=NORMAL_29" class="btn  <?=$scode == "NORMAL_29" ? "btn_03":"btn_02"?>  ft_11"><?="29 일권 ".$track_nor_29["cnt"]?>명</a>
        &nbsp;
        <a href="2024_event2_list.php?scode=STD_ALL" class="btn <?=$scode == "STD_ALL" ? "btn_03":"btn_02"?> ft_11"><?="학생 양일 ".$track_std_all["cnt"]?>명</a>
        <a href="2024_event2_list.php?scode=STD_28" class="btn <?=$scode == "STD_28" ? "btn_03":"btn_02"?> ft_11"><?="학생 28일 ".$track_std_28["cnt"]?>명</a>
        <a href="2024_event2_list.php?scode=STD_29" class="btn <?=$scode == "STD_29" ? "btn_03":"btn_02"?> ft_11"><?="학생 29일 ".$track_std_29["cnt"]?>명</a>
        &nbsp;
        |
        &nbsp;<a href="2024_event2_list.php?sc_md=free" class="btn <?=$sc_md == "free" ? "btn_03":"btn_02"?> ft_11">온라인   <?=$row_free_cnt["cnt"]?>명</a>
        <!-- <a href="2024_event2_wait_list.php" class="btn btn_02 ft_11">대기   <?=$row_wait_cnt["cnt"]?>명</a>-->
    ...
    <form class="form-inline" name="fsearch2" action="https://epiclounge.co.kr/v3/adm/2024_event2_export_excel.php" method="get">
