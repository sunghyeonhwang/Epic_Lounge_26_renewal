...
if(!empty($_GET[sc_text])){
	...
	$where .= " or category1 like '%{$_GET[sc_text]}%' ";
...
<form name="frm_delete" id="frm_delete" action="./rsc_review_list_proc.php" method="post">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" width="100px">카테고리1</th>
        <th scope="col" width="100px">카테고리2</th>
        <th scope="col" >제목</th>
        <th scope="col"  width="150px">난이도</th>
...
?>
    ...
    <tr class="bg0" id="tr_<?=$row_list["rsc_bbs_idx"]?>">
        ...
        <td><?=$row_list["category1"]?>
        <td><?=$row_list["category2"]?>
		</td>
        <td class="text_left">
...
<?
	...
	$result = sql_query($sql);
	$arr_engine = explode("|",$row_list["cate_product"]);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		$checked = false;
...
<?
	...
	$result = sql_query($sql);
	$arr_engine = explode("|",$row_list["cate_subject"]);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		$checked = false;
		foreach($arr_engine as $val){
			if(trim($val) == trim($row["rsc_name"])) $checked = true;
...
?>
				<li><input type="checkbox" name="subject_<?=$row_list["rsc_bbs_idx"]?>" id="subject_<?=$row_list["rsc_bbs_idx"]?>_<?=$i?>" <?=!$checked ? "":"checked='checked'"?> onchange="change_subject(<?=$row_list["rsc_bbs_idx"]?>,this)" value="<?=$row["rsc_name"]?>"><label for="subject_<?=$row_list["rsc_bbs_idx"]?>_<?=$i?>"><?=$row["rsc_name"]?></label></li>
<?
	}
?>
			</ul>
        ...
        <td>
            <select id="change_diff" name="change_diff"  onchange="change_difficult('<?=$row_list["rsc_bbs_idx"]?>',this)" style="width:50px;"> 
<?
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
?>
				<option value="<?=$row["rsc_name"]?>" <?=$row_list["cate_difficult"] == $row["rsc_name"] ? "selected='selected'":""?>><?=$row["rsc_name"]?></option>
<?
	}
?>
			</select>
...
<?
	...
	if($row_list["site_url"]){
		...
		$link = "/v3/contents/replay_view.php?idx=".$row_list["rsc_bbs_idx"];
	}
?>
        ...
        <td class="">
            <a href="./rsc_review_modify.php?rsc_bbs_idx=<?=$row_list["rsc_bbs_idx"]?>" class="btn btn_03 margin_b_5">수정</a><br />
...
<div class="btn_fixed_top">
        <input type="button" onclick="group_delete()" name="" value="선택삭제" onclick="" class="btn_02 btn">
            <a href="./rsc_review_write.php" id="bo_add" class="btn_01 btn">리소스 추가</a>
    </div>
...
<script>
    ...
    function move_to(mode, idx,field){
        $.ajax({
            ...
            data : {'mode':'review','idx': idx,'move':mode},
            dataType : 'json',
            success : function(json) {
