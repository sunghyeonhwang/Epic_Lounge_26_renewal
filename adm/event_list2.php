...
$sql_common = " from cb_unreal_2021_luman_apply a ";
...
<div class="tbl_head01 tbl_wrap">
    ...
    <th>신분증</th>
    <th>SNS</th>
    <th width="200" style="width:200px;" >작품소개</th>
    <th width="200"  style="width:200px;" >에셋출처</th>
    <th>T사이즈</th>
    <th>주소</th>
    <th>제출시간</th>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td><?=$row['apply_sns']?></td>
        <td><?=$row['apply_introduce']?></td>
        <td><?=$row['apply_source']?></td>
        <td><?=$row['t_size']?></td>
        <td><?=$row['addr_1']?><?=$row['addr_2']?><?=$row['addr_3']?></td>
...
<form class="form-inline" name="fsearch2" action="https://epiclounge.co.kr/v2/adm/export_excel_ch.php" method="get">
...
<script type="text/javascript">
    ...
    function del(val){
        if (confirm('삭제 하시겠습니까?')) {
            location.replace('/v3/event2023/findApplyByEmail.php?mode2=del&no='+val);
        }
    }
</script>
<?php
