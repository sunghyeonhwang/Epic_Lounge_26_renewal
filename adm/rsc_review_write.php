...
<form name="frm_bbs" id="frm_bbs" action="./rsc_review_write_proc.php" enctype="multipart/form-data" onsubmit="return submit_frm()" method="post">
...
<?
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
?>
					<li><input type="checkbox" name="cate_subject[]" id="rsc_<?=$row["rsc_idx"]?>" value="<?=$row["rsc_name"]?>" b<?=strpos($RData["cate_subject"], $row["rsc_name"]) === false ? "":"checked='checked'"?> /> <label for="rsc_<?=$row["rsc_idx"]?>"><?=$row["rsc_name"]?> </label></li>
<?
	}
?>
				</ul>
