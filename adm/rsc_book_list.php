...
if(!empty($_GET[sc_text])){
	...
	$where .= " or cate_product like '%{$_GET[sc_text]}%' ";
	$where .= " or cate_subject like '%{$_GET[sc_text]}%' ";
	$where .= " or cate_difficult like '%{$_GET[sc_text]}%' ";
	
	$where .= " ) ";

}
	$sql_list_cnt = 
		" select count(*) cnt from v3_rsc_book_bbs 
		  where 1=1 " 
		  . $where . " ";
...
<form name="frm_delete" id="frm_delete" action="./rsc_book_list_proc.php" method="post">
...
?>
    ...
    <tr class="bg0" id="tr_<?=$row_list["rsc_bbs_idx"]?>">
        ...
        <td>
            <select name="category" onchange="change_category('<?=$row_list["rsc_bbs_idx"]?>',this)" style="width:85px;">
				<option value="현장 가이드" <?=$row_list["category"] == "현장 가이드" ? "selected='selected'":""?>>현장 가이드</option>
				<option value="백서" <?=$row_list["category"] == "백서" ? "selected='selected'":""?>>백서</option>
			</select>
		</td>
        <td class="text_left">
...
<?
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
?>
				<li><input type="checkbox" name="product_<?=$row_list["rsc_bbs_idx"]?>" id="product_<?=$row_list["rsc_bbs_idx"]?>_<?=$i?>" <?=strpos($row_list["cate_product"], $row["rsc_name"]) === false ? "":"checked='checked'"?> onchange="change_product(<?=$row_list["rsc_bbs_idx"]?>,this)" value="<?=$row["rsc_name"]?>"><label for="product_<?=$row_list["rsc_bbs_idx"]?>_<?=$i?>"><?=$row["rsc_name"]?></label></li>
<?
	}
?>
			</ul>
...
<?
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
?>
				<li><input type="checkbox" name="subject_<?=$row_list["rsc_bbs_idx"]?>" id="subject_<?=$row_list["rsc_bbs_idx"]?>_<?=$i?>" <?=strpos($row_list["cate_subject"], $row["rsc_name"]) === false ? "":"checked='checked'"?> onchange="change_subject(<?=$row_list["rsc_bbs_idx"]?>,this)" value="<?=$row["rsc_name"]?>"><label for="subject_<?=$row_list["rsc_bbs_idx"]?>_<?=$i?>"><?=$row["rsc_name"]?></label></li>
<?
	}
?>
			</ul>
...
<?
	...
	if($row_list["site_url"]){
		...
		$link = "/v3/contents/book_view.php?idx=".$row_list["rsc_bbs_idx"];
	}
?>
        ...
        <td class="">
            <a href="./rsc_book_modify.php?rsc_bbs_idx=<?=$row_list["rsc_bbs_idx"]?>" class="btn btn_03 margin_b_5">수정</a><br />
...
<div class="btn_fixed_top">
        <input type="button" onclick="group_delete()" name="" value="선택삭제" onclick="" class="btn_02 btn">
            <a href="./rsc_book_write.php" id="bo_add" class="btn_01 btn">리소스 추가</a>
    </div>
...
<script>
    ...
    function move_to(mode, idx,field){
        $.ajax({
            ...
            data : {'mode':'book','idx': idx,'move':mode},
            dataType : 'json',
            success : function(json) {
