...
<form name="frm_bbs" id="frm_bbs" action="./rsc_book_write_proc.php" enctype="multipart/form-data" onsubmit="return submit_frm()" method="post">
<div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="category"><label for="category">카테고리</label></th>
            <td colspan="2">
...
<?
	...
	$sql = " select * from v3_rsc_book_category where rsc_type='산업분야' order by rsc_idx desc ";
	$result = sql_query($sql);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
?>
					<li><input type="checkbox" name="cate_industry[]" id="rsc_<?=$row["rsc_idx"]?>" value="<?=$row["rsc_name"]?>" /> <label for="rsc_<?=$row["rsc_idx"]?>"><?=$row["rsc_name"]?> </label></li>
<?
	}
?>
				</ul>
...
<?
	...
	$sql = " select * from v3_rsc_book_category where rsc_type='제품군' order by rsc_idx desc ";
	$result = sql_query($sql);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
?>
					<li><input type="checkbox" name="cate_product[]" id="rsc_<?=$row["rsc_idx"]?>" value="<?=$row["rsc_name"]?>" /> <label for="rsc_<?=$row["rsc_idx"]?>"><?=$row["rsc_name"]?> </label></li>
<?
	}
?>
				</ul>
...
<?
	...
	$sql = " select * from v3_rsc_book_category where rsc_type='주제' order by rsc_idx desc ";
	$result = sql_query($sql);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
?>
					<li><input type="checkbox" name="cate_subject[]" id="rsc_<?=$row["rsc_idx"]?>" value="<?=$row["rsc_name"]?>" /> <label for="rsc_<?=$row["rsc_idx"]?>"><?=$row["rsc_name"]?> </label></li>
<?
	}
?>
				</ul>
