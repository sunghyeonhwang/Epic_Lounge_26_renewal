...
//문자 발송 종료
    ...
    $sql = "select count(*) from cb_unreal_2025_event2_apply where apply_user_email = '$apply_user_email' and apply_no != '$apply_no' limit 1";
    $result = $mysqli -> query($sql);
    $obj = $result -> fetch_array();
    if($obj[0]['count']){
        echo '<script type="text/javascript">';
    ...
    $sql = "select count(*) from cb_unreal_2025_event2_apply where apply_user_phone = '$apply_user_phone' and apply_no != '$apply_no' limit 1";
    $result = $mysqli -> query($sql);
    $obj = $result -> fetch_array();
    if($obj[0]['count']){
        echo '<script type="text/javascript">';
  ...
  WHERE apply_no = '$apply_no'";
    ...
    $mysqli -> close();
    echo '<script type="text/javascript">';
    echo "location.replace('/v3/adm/2025_event2_list.php');";
    echo '</script>';
    exit();
    ...
    sql_query("insert into 2025_event2_log(log_text,rdate) values('".str_replace("'","`",$response)."',now())");

    if($resultMap["resultCode"] == "01"){

        echo '<script type="text/javascript">';
        echo "alert('".$resultMap["resultMsg"]."');";
        echo "alert('환불에 실패하였습니다.'); location.replace('/v3/adm/2025_event2_list.php');";
        echo '</script>';
        exit();
    }else{
  ...
  WHERE apply_no = '$apply_no'";
        sql_query($sql2);
        echo '<script type="text/javascript">';
        echo "alert('환불이 완료되었습니다.'); location.replace('/v3/adm/2025_event2_list.php');";
        echo '</script>';
        exit();
    }
    ...
    $mysqli = new mysqli("localhost","unrealengine","Good121930!@","unrealengine");
    $sql = "INSERT INTO cb_unreal_2025_event2_apply_inquire(
    apply_user_name, 
    apply_user_email,
    apply_user_phone,
    apply_user_company, 
