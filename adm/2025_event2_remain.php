        ...
        <div class="tbl_frm01 tbl_wrap">
            ...
            <table class="btn_table_re" style="width:1400px;">
                ...
                <tr >
                    ...
                    <th >카운트</th>
                    <th >현재설정값</th><!--
                    <th style="text-align: right;" colspan="2">26일</th>
                    <th >카운트</th>
                    <th >현재설정값</th>-->
                </tr>
                </thead>
                ...
                while($row_track = sql_fetch_array($result_track)){
                    ...
                    $sql = "select count(*) as cnt from cb_unreal_2025_event2_apply where apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_reg_datetime <= '2025-07-23 23:59:59' and apply_coupon_no = 0  and apply_track like '%".$track."%'";
                    $track_discount_25 = sql_fetch($sql);

                    $sql = "select count(*) as cnt from cb_unreal_2025_event2_apply where apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_reg_datetime >= '2025-07-23 23:59:59' and apply_coupon_no = 0  and apply_track like '%".$track."%'";
                    $track_nor_25 = sql_fetch($sql);

                    $sql = "select count(*) as cnt from cb_unreal_2025_event2_apply where apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_coupon_no <> 0 and apply_coupon_no < 10000  and apply_track like '%".$track."%'";
                    $track_coupon_25 = sql_fetch($sql);

                    ?>
                    <?php
                    $total = $track_nor_25["cnt"] + $track_discount_25["cnt"] + $track_coupon_25["cnt"];
                    ?>
                    <form action="2025_event2_remain_proc.php" method="post">
                        <input type="hidden" name="name" value="<?=$row_track["name"]?>">
                        <tr class="last">
                            <td style="text-align: right;" ><?=$row_track["label"]?><br /><?=$row_track["name"]?></td>
                            <td style="text-align: right;"> <span style="font-weight: bold;"><?=$total?></span>  (얼리버드: <?=$track_discount_25["cnt"]?>/일반:<?=$track_nor_25["cnt"]?>/단체:<?=$track_coupon_25["cnt"]?>)/<?=$row_track["date1"]?></td>
                            <td>잔여 : <?=$row_track["date1"] - ($track_discount_25["cnt"] + $track_nor_25["cnt"] + $track_coupon_25["cnt"])?></td>
...
<!--
                            <td style="text-align: right;"><?php /*=$track_nor_26["cnt"] + $track_std_26["cnt"]*/?>(일반:<?php /*=$track_nor_26["cnt"]*/?>)/<?php /*=$row_track["date2"]*/?></td>
                            <td>잔여 : <?php /*=$row_track["date2"] - ($track_nor_26["cnt"] + $track_std_26["cnt"])*/?></td>

                            <td style="background-color: #CCECFF"><input type="text" name="date2" value="<?php /*=$row_track["date2"]*/?>" id="stx" class=" frm_input" >
                                <input type="submit" class="btn btn_02" value="변경하기" /></td>
                            <td ><?php /*=$row_track["date2"]*/?></td>-->
                        </tr>
                    </form>
                    <?
                }
