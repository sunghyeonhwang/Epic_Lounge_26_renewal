...
<div class="local_ov01 local_ov">
  <span class="btn_ov01"><span class="ov_txt">생성된 새소식수</span><span class="ov_num"> <?=$cnt["cnt"]?>개</span></span>
</div>
...
<form name="frm_delete" id="frm_delete" action="./rsc_news_list_proc.php" method="post">
...
?>
    ...
    <tr class="bg0" id="tr_<?=$row_list["rsc_bbs_idx"]?>">
        ...
        <td class="text_left"><?=$row_list["tag"]?></td>
...
<?
	...
	if($row_list["site_url"]){
		...
		$link = "/v3/contents/news_view.php?idx=".$row_list["rsc_bbs_idx"];
	}
?>
        ...
        <td class="">
            <a href="./rsc_news_modify.php?rsc_bbs_idx=<?=$row_list["rsc_bbs_idx"]?>" class="btn btn_03 margin_b_5">수정</a><br />
...
<div class="btn_fixed_top">
        <input type="button" onclick="group_delete()" name="" value="선택삭제" onclick="" class="btn_02 btn">
            <a href="./rsc_news_write.php" id="bo_add" class="btn_01 btn">새소식 추가</a>
    </div>
...
function group_delete(){
	if(confirm("정말로 선택한 새소식를 삭제 하시겠습니까?")){
		$("#frm_delete").submit();
	}
}
$(function(){
...
<script>
    ...
    function move_to(mode, idx,field){
        $.ajax({
            ...
            data : {'mode':'news','idx': idx,'move':mode},
            dataType : 'json',
            success : function(json) {
