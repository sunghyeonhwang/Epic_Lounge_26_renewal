...
// 기간별(등록일) 검색 — YYYY-MM-DD 형식만 허용
$ds = (isset($_GET['ds']) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['ds'])) ? $_GET['ds'] : '';
$de = (isset($_GET['de']) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['de'])) ? $_GET['de'] : '';
// 기간(등록일) 조건 — 목록 WHERE 와 현황(cnt2) 통계에 공통 적용
$dcond = '';
if ($ds !== '') { $dcond .= " AND apply_reg_datetime >= '".sql_real_escape_string($ds)." 00:00:00'"; }
if ($de !== '') { $dcond .= " AND apply_reg_datetime <= '".sql_real_escape_string($de)." 23:59:59'"; }
...
if ($q !== '') {
    ...
    $where .= " AND (apply_user_name LIKE '%$qe%' OR apply_user_email LIKE '%$qe%' OR apply_user_phone LIKE '%$qe%' OR apply_user_company LIKE '%$qe%')";
}
// 기간별(등록일) 필터
$where .= $dcond;
...
if (isset($_POST['delete_no'])) {
    ...
    $rp = isset($_GET['p']) ? max(1,(int)$_GET['p']) : 1;
    header('Location: 2026_event2_list.php?'.http_build_query(array('q'=>$q,'ds'=>$ds,'de'=>$de,'st'=>$st,'p'=>$rp,'done'=>'delete')));
    exit;
}
...
function cnt2($w){ global $dcond; $r = sql_fetch("SELECT count(*) c FROM cb_unreal_2026_event2_apply WHERE $w".$dcond); return $r ? (int)$r['c'] : 0; }
...
<div class="ufs26">
  <h1>UFS 2026 등록 현황<?php if ($ds !== '' || $de !== ''): ?> <span style="font-size:13px;font-weight:700;color:#7c3aed;background:#f3effe;border:1px solid #ddd0f7;border-radius:6px;padding:3px 9px;vertical-align:middle">📅 <?= e2($ds!==''?$ds:'처음') ?> ~ <?= e2($de!==''?$de:'현재') ?> 기준</span><?php endif; ?></h1>

  <div class="cards cards-sum">
    <a class="card<?= $st==='all'?' on':'' ?>" href="?<?= e2(http_build_query(array('ds'=>$ds,'de'=>$de,'st'=>'all'))) ?>"><div class="lbl">총 등록</div><div class="num"><?= number_format($stat['offline']+$stat['online']) ?></div></a>
    <a class="card<?= $st==='offline'?' on':'' ?>" href="?<?= e2(http_build_query(array('ds'=>$ds,'de'=>$de,'st'=>'offline'))) ?>"><div class="lbl">오프라인 합계</div><div class="num" style="color:#00897B"><?= number_format($stat['offline']) ?></div></a>
    <a class="card<?= $st==='online'?' on':'' ?>" href="?<?= e2(http_build_query(array('ds'=>$ds,'de'=>$de,'st'=>'online'))) ?>"><div class="lbl">온라인 합계</div><div class="num" style="color:#2aa7b5"><?= number_format($stat['online']) ?></div></a>
  </div>
  <div class="cards">
    <a class="card<?= $st==='allday'?' on':'' ?>" href="?<?= e2(http_build_query(array('ds'=>$ds,'de'=>$de,'st'=>'allday'))) ?>"><div class="lbl">오프라인(양일권)</div><div class="num" style="color:#00C1D5"><?= number_format($stat['allday']) ?></div></a>
    <a class="card<?= $st==='day1'?' on':'' ?>" href="?<?= e2(http_build_query(array('ds'=>$ds,'de'=>$de,'st'=>'day1'))) ?>"><div class="lbl">오프라인 20일</div><div class="num" style="color:#307FE2"><?= number_format($stat['day1']) ?></div></a>
    <a class="card<?= $st==='day2'?' on':'' ?>" href="?<?= e2(http_build_query(array('ds'=>$ds,'de'=>$de,'st'=>'day2'))) ?>"><div class="lbl">오프라인 21일</div><div class="num" style="color:#DD0AB2"><?= number_format($stat['day2']) ?></div></a>
    <a class="card<?= $st==='online'?' on':'' ?>" href="?<?= e2(http_build_query(array('ds'=>$ds,'de'=>$de,'st'=>'online'))) ?>"><div class="lbl">온라인</div><div class="num" style="color:#2aa7b5"><?= number_format($stat['online']) ?></div></a>
    <a class="card<?= $st==='group'?' on':'' ?>" href="?<?= e2(http_build_query(array('ds'=>$ds,'de'=>$de,'st'=>'group'))) ?>"><div class="lbl">단체</div><div class="num" style="color:#7c3aed"><?= number_format($stat['group']) ?></div></a>
    <a class="card<?= $st==='cancel'?' on':'' ?>" href="?<?= e2(http_build_query(array('ds'=>$ds,'de'=>$de,'st'=>'cancel'))) ?>"><div class="lbl">취소</div><div class="num" style="color:#9aa0af"><?= number_format($stat['cancel']) ?></div></a>
  </div>
  ...
  <div id="viewList">
    <form method="get" class="toolbar">
      <input type="text" name="q" value="<?= e2($q) ?>" placeholder="이름·이메일·연락처·회사 검색">
      <input type="date" name="ds" value="<?= e2($ds) ?>" title="등록일 시작" style="width:150px" placeholder="시작일">
      <span style="color:#8a90a2;font-size:13px">~</span>
      <input type="date" name="de" value="<?= e2($de) ?>" title="등록일 종료" style="width:150px" placeholder="종료일">
      <select name="st">
      ...
      <a href="<?= $self ?>?<?= e2(http_build_query(array('q'=>$q,'ds'=>$ds,'de'=>$de,'st'=>$st,'export'=>1))) ?>" class="btn">CSV 내보내기</a>
    ...
    <div style="background:rgba(0,193,213,.08);border:1px solid rgba(0,193,213,.35);color:#cfd3dc;padding:12px 16px;border-radius:8px;margin-bottom:12px">
      <b style="color:#fff"><?= e2($er['apply_user_name']) ?></b> 참석일 변경 <span style="color:#8a90a2;font-size:12px">· 1일권(8.20↔8.21)만 · 가격 동일(정산 없음)</span>
      <form method="post" action="2026_event2_list.php?<?= e2(http_build_query(array('q'=>$q,'ds'=>$ds,'de'=>$de,'st'=>$st,'p'=>$page))) ?>" style="margin-top:10px;display:flex;gap:10px;align-items:center;flex-wrap:wrap">
        <input type="hidden" name="change_day" value="1">
        <input type="hidden" name="change_no" value="<?= $eno ?>">
        <label>참석일 <select name="newday" id="edDay" onchange="edFillTracks()" style="padding:5px;margin-left:4px">
          <option value="1"<?= $ecurday===1?' selected':'' ?>>8월 20일 (Day1)</option>
          <option value="2"<?= $ecurday===2?' selected':'' ?>>8월 21일 (Day2)</option>
        </select></label>
        <label>트랙 <select name="newtrack" id="edTrack" style="padding:5px;margin-left:4px"></select></label>
        <button type="submit" class="btn pri" style="padding:6px 14px">변경 저장</button>
        <a href="2026_event2_list.php?<?= e2(http_build_query(array('q'=>$q,'ds'=>$ds,'de'=>$de,'st'=>$st,'p'=>$page))) ?>" style="color:#8a90a2">취소</a>
      </form>
      <script>
      var ED_TR={"1":{"DAY1_TR1":"게임: 프로그래밍","DAY1_TR2":"게임: 아트","DAY1_TR3":"미디어 & 엔터테인먼트","DAY1_TR4":"공통"},
                 "2":{"DAY2_TR1":"게임: 프로그래밍","DAY2_TR2":"게임: 아트","DAY2_TR3":"미디어 & 엔터테인먼트","DAY2_TR4":"제조 및 시뮬레이션"}};
      var ED_CUR={day:"<?= $ecurday ?>",track:<?= json_encode($ecurtrack, JSON_UNESCAPED_UNICODE) ?>};
      function edFillTracks(){
        var d=document.getElementById('edDay').value, sel=document.getElementById('edTrack'), tr=ED_TR[d]||{};
        while(sel.firstChild){ sel.removeChild(sel.firstChild); }
        for(var k in tr){ var o=document.createElement('option'); o.value=k; o.text=tr[k]+' ('+k+')'; if(d===ED_CUR.day && k===ED_CUR.track){ o.selected=true; } sel.appendChild(o); }
      }
      edFillTracks();
      </script>
    </div>
    <?php elseif ($er && !$er_1day): ?>
    <div style="background:rgba(255,143,28,.12);border:1px solid rgba(255,143,28,.4);color:#FF8F1C;padding:10px 14px;border-radius:8px;margin-bottom:12px;font-size:13px">개인 오프라인 1일권만 참석일을 변경할 수 있습니다. (양일권·온라인·단체는 불가)</div>
    <?php elseif ($er && !$er_ebok): ?>
    <div style="background:rgba(255,143,28,.12);border:1px solid rgba(255,143,28,.4);color:#FF8F1C;padding:10px 14px;border-radius:8px;margin-bottom:12px;font-size:13px">현재 판매 단계와 맞지 않아 변경할 수 없습니다. (얼리버드 신청은 얼리버드 기간에만, 일반판매분은 종료 후에만)</div>
    <?php endif; ?>
    <?php endif; ?>
    ...
    <?php elseif (isset($_GET['done']) && $_GET['done']==='daychange'): ?>
    <div style="background:rgba(0,193,213,.12);border:1px solid rgba(0,193,213,.4);color:#00C1D5;padding:10px 14px;border-radius:8px;margin-bottom:12px;font-size:13px">참석일을 변경했습니다. (1일권 · 가격 동일)</div>
    <?php elseif (isset($_GET['done']) && $_GET['done']==='daychange_ebblock'): ?>
    ...
    <table>
      ...
      <?php if ($list && $list->num_rows): while ($r = $list->fetch_assoc()):
        ...
        <tr>
          ...
          <td style="white-space:nowrap">
            ...
            <?php elseif ($ps !== 0): ?>
              <form method="post" action="2026_event2_list.php?<?= e2(http_build_query(array('q'=>$q,'ds'=>$ds,'de'=>$de,'st'=>$st,'p'=>$page))) ?>" style="display:inline" onsubmit="return confirm('[<?= e2($r['apply_user_name']) ?>] 님의 등록을 취소 처리할까요?\n유료 결제는 INICIS에서 자동 환불되고 상태가 취소로 바뀝니다. (환불 실패 시 취소되지 않음)');">
          ...
          <td style="white-space:nowrap">
            <form method="post" action="2026_event2_list.php?<?= e2(http_build_query(array('q'=>$q,'ds'=>$ds,'de'=>$de,'st'=>$st,'p'=>$page))) ?>" style="display:inline" onsubmit="return confirm('[<?= e2($r['apply_user_name']) ?>] 등록을 완전히 삭제할까요?\n삭제하면 복구할 수 없습니다.');">
      ...
      <?php endwhile; else: ?>
        <tr><td colspan="14" style="padding:40px;text-align:center;color:#9aa0af">등록 내역이 없습니다.</td></tr>
      <?php endif; ?>
      </tbody>
    </table>
    </div>
    ...
    <div class="pg">
      <?php for ($i=1;$i<=$pages;$i++): ?>
        <a href="<?= $self ?>?<?= e2(http_build_query(array('q'=>$q,'ds'=>$ds,'de'=>$de,'st'=>$st,'p'=>$i))) ?>" class="<?= $i===$page?'on':'' ?>"><?= $i ?></a>
      <?php endfor; ?>
    </div>
    <?php endif; ?>
  </div>
...
function ufsBulkDelete(){
  ...
  f.action='2026_event2_list.php?<?= e2(http_build_query(array('q'=>$q,'ds'=>$ds,'de'=>$de,'st'=>$st,'p'=>$page))) ?>';
