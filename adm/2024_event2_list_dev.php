...
$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'NORMAL%'  and (apply_product_code like '%ALL')";
$track_nor_all = sql_fetch($sql);

$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'STD%'  and ( apply_product_code like '%ALL')";
$track_std_all = sql_fetch($sql);

$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'NORMAL%'  and (apply_product_code like '%28')";
$track_nor_28 = sql_fetch($sql);

$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'STD%'  and (apply_product_code like '%28')";
$track_std_28 = sql_fetch($sql);


$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'NORMAL%'  and (apply_product_code like '%29')";
$track_nor_29 = sql_fetch($sql);

$sql = "select count(*) as cnt from cb_unreal_2024_event2_apply where apply_pay_status <> 0 and apply_temp_yn = 'N' and apply_product_code like 'STD%'  and (apply_product_code like '%29')";
...
?>
	...
	<div class="local_ov01 local_ov">
        ...
        <strong>오프라인</strong>: 28일권 <strong><?=($track_nor_28["cnt"] + $track_nor_all["cnt"] + $track_std_28["cnt"] + $track_std_all["cnt"])?></strong>명/1120명 (일반:<?=$track_nor_28["cnt"] + $track_nor_all["cnt"]?>명, 학생:<?=$track_std_28["cnt"] + $track_std_all["cnt"]?>명), 29일권 <?=($track_nor_29["cnt"] + $track_nor_all["cnt"] + $track_std_29["cnt"] + $track_std_all["cnt"])?> 명/1120명 (일반:<?=$track_nor_29["cnt"] + $track_nor_all["cnt"]?>명, 학생:<?=$track_std_29["cnt"] + $track_std_all["cnt"]?>명)</span><br><br>
