...
function url_auto_link($str)
{
    ...
    $str = preg_replace("/([^(href=\"?'?)|(src=\"?'?)]|\(|^)((http|https|ftp|telnet|news|mms):\/\/[a-zA-Z0-9\.-]+\.[\xea\xb0\x80-\xed\x9e\xa3\xa1-\xfea-zA-Z0-9\.:&#!=_\?\/~\+%@;\-\|\,\(\)]+)/i", "\\1<A HREF=\"\\2\" TARGET=\"{$config['cf_link_target']}\" $attr_nofollow>\\2</A>", $str);
...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id || (isset($member['mb_id']) && $member['mb_id'] == $mb_id && (int) $member['mb_level'] >= 10)) {
        $is_authority = 'super';
...
function delete_editor_thumbnail($contents)
{
    ...
    for ($i = 0; $i < count($matchs[1]); $i++) {
        ...
        if (!preg_match('/(\.jpe?g|\.gif|\.png|\.webp)$/i', $srcfile))
            continue;
...
function check_vaild_callback($callback)
{
     ...
     */
    if (substr($_callback, 0, 4) == '1588')
        if (strlen($_callback) != 8)
            return false;
    if (substr($_callback, 0, 2) == '02')
        if (strlen($_callback) != 9 && strlen($_callback) != 10)
            return false;
    if (substr($_callback, 0, 3) == '030')
        if (strlen($_callback) != 10 && strlen($_callback) != 11)
            return false;
